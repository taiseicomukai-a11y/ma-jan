...
export const DIFFICULTIES: DifficultyConfig[] = [
  { id: "beginner", label: "初級 4枚", handSize: 4, targetMelds: 1, enabled: true },
  { id: "intermediate", label: "中級 7枚", handSize: 7, targetMelds: 2, enabled: true },
  // TODO: 10枚、13/14枚対応では targetMelds を増やすだけでなく、問題生成の品質を調整する。
  { id: "advanced", label: "上級 10枚", handSize: 10, targetMelds: 3, enabled: false }
];
...
export function canAddTile(counts: TileCounts, tile: Tile): boolean {
  return counts[tileIndex(tile)] < 4;
}
...
export function winningTilesFromCounts(counts: TileCounts, targetMelds: number): Tile[] {
  return ALL_TILES.filter((tile) => {
    const index = tileIndex(tile);
    if (counts[index] >= 4) return false;
    const next = [...counts];
    next[index] += 1;
    return isWinningCounts(next, targetMelds);
  });
}
...
export function describeImprovement(discard: Tile, effectiveTiles: Tile[], count: number): string {
  ...
  return `${discard}切りは ${names} の${effectiveTiles.length}種類を受けられ、残り枚数ベースで${count}枚あります。形を固定しすぎず、次ツモで完成形に近いまとまりを作れるのが強みです。`;
}
